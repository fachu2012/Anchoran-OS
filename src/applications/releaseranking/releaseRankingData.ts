...
export const RANKING_ROWS: RankingRow[] = [
  ...
  ["3.1.2", "2026-09-13", 9, "The real fix for the Insider-Preview-can't-detect-stable bug: the internal semver suffix an I.P.U. build's version.json carries is now \"-beta\" instead of \"-IPU\", which is what electron-updater's own logic actually recognizes as a walkable prerelease channel (nothing user-facing changes — the tag, release name and \"I.P.U.\" badge stay the same). Also narrows the full \"Version # | Build #H#.#\" label down to exactly three places (the release name, the update-ready screen's title, Anchover); everywhere else now uses a compact \"Build-#H#.#\" form.", "no known open bugs at this point"],
  ["3.3.6", "2026-09-13", 9, "Third-party plugin apps land: a new Anchoran App SDK (<code class='inline'>window.AnchoranSDK</code>) and a privileged <code class='inline'>anchoran-plugin://</code> protocol let the Webstore's new Community section download and run real plugin apps from a separate, independently-versioned repo — no Anchoran OS release needed to publish or update one. The Webstore now also shows Anchoran OS's own version in its sidebar.", "no known open bugs at this point — the plugin system itself is a new, deliberately small pilot (one SDK version, one example plugin), not yet broadly exercised"],
