...
export const RANKING_ROWS: RankingRow[] = [
  ["3.8.0", "2026-09-14", 9, "A deliberate jump from v3.5.6 (skipping v3.6/v3.7 on purpose): a fullscreen BIOS-style boot confirmation before anything else; Anchoran now actually hides and throttles every other Windows app on its own monitor (never a second monitor); System Mode's toggle/Terminal command are both gone — unconditional now; every real exit ends with 2s of black screen; and a crash watchdog running as a genuinely separate process replaces the old in-process React error screen with one consistent crash screen (real Terminal, no PIN, Restart/Force close) for any kind of crash, not just React's own.", "no known open bugs at this point — still I.P.U.-only as of this update"],
