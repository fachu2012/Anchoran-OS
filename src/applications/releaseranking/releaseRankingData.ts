...
export const RANKING_ROWS: RankingRow[] = [
  ["3.8.3", "2026-09-15", 9, "Critical fix: the Windows key stopped opening the Launcher on every version since v3.8.1 — kioskhook talked to Anchoran over stdout/stdin, and routing it through the new <code class='inline'>AnchoranLauncher.exe</code> (to survive a Task Manager grouped \"End task\") tried to forward stdio two hops deep, which didn't reliably carry its output back. kioskhook now talks to Anchoran over its own dedicated named pipe instead, the same pattern the crash watchdog already used; the launcher itself no longer attempts any stdio forwarding, since nothing needs it anymore.", "no known open bugs at this point — still I.P.U.-only as of this update"],
