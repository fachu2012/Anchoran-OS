...
export const RANKING_ROWS: RankingRow[] = [
  ["3.8.6", "2026-09-15", 8, "A batch of real bugs found via live testing on v3.8.5, plus a redesign of the crash screen and how the watchdog reacts: a freshly-set admin PIN could leave \"Run as Administrator\" saying no admin account had one configured at all, if you never switched profiles since setting it (reproduced on a brand new install, setting the owner's PIN through the setup wizard) — <code class='inline'>profiles</code> now stays in sync with the active profile's live edits continuously, not just at switch time; every normal, deliberate way to close Anchoran (Shut down, \"n\" on the boot confirmation, Alt+F4) still made the watchdog show its crash screen, since it couldn't tell that apart from a real crash — Anchoran now tells it beforehand; and kioskhook's <code class='inline'>EnumWindows</code>/keyboard-hook callbacks now catch and skip past errors individually instead of one taking the whole process down. The watchdog also now keeps an always-ready hidden black \"curtain\" window for a near-instant crash reveal (covering the real crash screen's own few-second Electron cold-start, measured at up to 6s bare desktop otherwise), and the crash screen itself is redesigned closer to Windows' own \"stop\" screen in layout (big \":(\", plain-language headline) while staying Anchoran's own dark theme — the Terminal is now hidden by default behind an \"Open Anchoran Terminal\" button, and the buttons are renamed to \"Auto-Repair Anchoran\" and \"Exit Anchoran\".", "no known open bugs at this point — still I.P.U.-only as of this update"],
  ["3.8.5", "2026-09-15", 5, "Two more real bugs found via live testing on v3.8.4: the Windows key stopped opening the Launcher again right after v3.8.4 shipped — its new Progman/WorkerW check used an unsafe P/Invoke binding that could crash kioskhook entirely from inside EnumWindows' own callback, fixed by switching to the same plain DllImport+StringBuilder pattern used everywhere else; and the crash watchdog's screen turned out to have never worked at all, on any version since it went native in v3.8.1 — its named pipe was created read-only from its own side, which silently rejects the full read+write connection Node always opens, so <code class='inline'>anchoran testcrash 1</code> could never have shown it no matter what. Also: the lone Anchoran icon is now always the classic brand blue everywhere it appears solo (shutdown, both update screens, the installer's own progress bar), not just the boot screen.", "3 open, all found via live testing on this exact version: a fresh admin PIN could leave no admin account recognized at all if you never switched profiles (real fix v3.8.6); every normal deliberate close (Shut down, boot-screen \"n\", Alt+F4) still wrongly triggered the crash screen (real fix v3.8.6, a new graceful-shutdown signal); and the Windows key could still occasionally stop working from an unhandled error elsewhere in the same native callbacks (real fix v3.8.6, broader defensive hardening)"],
  ["3.8.4", "2026-09-15", 5, "Two more real bugs found via live testing on v3.8.3: the desktop background (and any Lively Wallpaper running) never came back after Anchoran closed, by any exit path — kioskhook was hiding Explorer's own Progman/WorkerW windows for no actual benefit (Anchoran's own fullscreen window already covers them the entire time it runs) and Windows doesn't always repaint them cleanly after a hide/show cycle, so they're now never touched at all; and a crash message could be silently dropped before Anchoran quit if the watchdog's pipe wasn't connected yet, closing Anchoran with no crash screen at all (<code class='inline'>anchoran testcrash 1</code> reproduced this) — reporting a crash now always waits for a real send attempt first.", "2 open, both found via live testing on this exact version: the new Progman/WorkerW check's own P/Invoke binding could crash kioskhook entirely, breaking the Windows key again (real fix v3.8.5); and the crash watchdog's pipe was read-only from its own side the whole time, so its screen had actually never worked, ever — not something this version broke, just never noticed until testcrash existed to probe it (real fix v3.8.5)"],
  ["3.8.3", "2026-09-15", 6, "Critical fix: the Windows key stopped opening the Launcher on every version since v3.8.1 — kioskhook talked to Anchoran over stdout/stdin, and routing it through the new <code class='inline'>AnchoranLauncher.exe</code> (to survive a Task Manager grouped \"End task\") tried to forward stdio two hops deep, which didn't reliably carry its output back. kioskhook now talks to Anchoran over its own dedicated named pipe instead, the same pattern the crash watchdog already used; the launcher itself no longer attempts any stdio forwarding, since nothing needs it anymore.", "2 open, both found via live testing on this exact version: the desktop background/Lively Wallpaper never came back after Anchoran closed (real fix v3.8.4), and a renderer crash could close Anchoran with no crash screen at all if the watchdog's pipe wasn't connected yet (real fix v3.8.4)"],
  ["3.8.2", "2026-09-15", 5, "A cosmetic follow-up to v3.8.1's boot screen: the icon no longer visibly shifts position when the loading bar appears or disappears (both are now always in the layout, toggled only via opacity), and the whole sequence is more deliberately staged — black (3s), icon alone (5s), bar at 0% (3s), a slow uneven fill with random pauses (5-8s), the full bar held visible (2s), then a hard cut back to black (3s) before fading into the lock screen.", "1 open, serious: the Windows key silently stopped opening the Launcher (inherited unnoticed from v3.8.1, only found and fixed in v3.8.3) — kioskhook's stdout stopped reliably reaching Anchoran once it was spawned through AnchoranLauncher.exe"],
  ["3.8.1", "2026-09-14", 4, "Follow-up to v3.8.0's Windows-independence bundle: <code class='inline'>AnchoranLauncher.exe</code> now starts kioskhook and the crash watchdog reporting explorer.exe, not Anchoran, as their parent, so both survive a Task Manager grouped \"End task\" on Anchoran itself; the watchdog is now its own native executable instead of reusing Anchoran's packaged Electron binary; the BIOS-style boot confirmation is now a real staged sequence with typed y/n input instead of an instant single-keystroke prompt; the normal boot screen now stages like a real Windows boot (abrupt icon, uneven bar fill) and the lone Anchoran icon is always the classic brand blue regardless of accent; and <code class='inline'>anchoran testcrash</code> lets an admin deliberately trigger 5 different real crash types to test the watchdog on demand.", "2 open: the Windows key silently stopped opening the Launcher — this exact version's own AnchoranLauncher.exe broke kioskhook's stdout reaching Anchoran (real fix v3.8.3, a serious regression to a core interaction); and a cosmetic one, the rewritten boot screen's icon visibly shifting position when the loading bar appeared/disappeared (closes v3.8.2)"],
  ["3.8.0", "2026-09-14", 7, "A deliberate jump from v3.5.6 (skipping v3.6/v3.7 on purpose): a fullscreen BIOS-style boot confirmation before anything else; Anchoran now actually hides and throttles every other Windows app on its own monitor (never a second monitor); System Mode's toggle/Terminal command are both gone — unconditional now; every real exit ends with 2s of black screen; and a crash watchdog running as a genuinely separate process replaces the old in-process React error screen with one consistent crash screen (real Terminal, no PIN, Restart/Force close) for any kind of crash, not just React's own.", "3 open, all confirmed by live testing: force-closing Anchoran via Task Manager's grouped \"End task\" killed the watchdog AND kioskhook right along with it, since Windows still showed both nested under Anchoran's own process tree despite being spawned detached (real fix v3.8.1); a main-process uncaught exception never reached the watchdog at all, just a disk log (real fix v3.8.1); a native renderer crash (not a React error) was never detected either (real fix v3.8.1)"],
  ["3.5.6", "2026-09-14", 9, "The real fix for the v3.5.4/v3.5.5 boot crash below: <code class='inline'>semver</code> (used by the new custom update provider) was only ever a transitive dependency, so electron-builder never packaged it into the installed app — now a direct dependency.", "no known open bugs at this point — never got a stable release of its own; folds into v3.8.0 instead"],
  ["3.5.5", "2026-09-14", 1, "<b>Fails to boot.</b> A trivial version bump on top of everything v3.5.4 shipped — no functional changes of its own — but inherits v3.5.4's packaging bug unchanged.", "guaranteed crash on startup (<code class='inline'>Cannot find module 'semver'</code>) — real fix in v3.5.6"],
  ["3.5.4", "2026-09-14", 1, "<b>Fails to boot.</b> On top of everything v3.5.0–v3.5.3 already shipped: a custom update provider fixes I.P.U.-to-I.P.U. auto-detection (electron-updater's own logic could only ever find the next stable release from an I.P.U.); a per-plugin \"Auto-update\" toggle updates a Webstore plugin right before it opens, with an \"Updating…\" screen; <code class='inline'>anchoran apps/uninstall/open/install</code> now see Webstore plugins, not just bundled apps; a plugin installed before manifest.json existed (e.g. Anchoran Code Studio) no longer shows up in the Launcher as its raw id; Code Studio can be uninstalled like any other plugin; and the Browser's secondary toolbar moves into a \"⋮\" menu.", "guaranteed crash on startup: <code class='inline'>semver</code> (used by the new update provider) was never packaged into the installed app since it wasn't a direct dependency — closes in v3.5.6"],
  ["3.5.3", "2026-09-14", 9, "On top of everything v3.5.0–v3.5.2 already shipped: fixes <code class='inline'>anchoran changeto</code> 404ing on every I.P.U. install and the taskbar not reappearing over a maximized window; a plugin can now request its own window size (<code class='inline'>preferredSize</code>); installed Webstore plugins now show up in the Launcher (and disappear on uninstall); and the taskbar's right-click menu always offers \"New window\" first.", "no known open bugs at this point — never got a stable release of its own; folds into v3.5.6 instead"],
