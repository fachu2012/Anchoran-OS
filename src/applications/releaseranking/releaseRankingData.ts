...
export const RANKING_ROWS: RankingRow[] = [
  ...
  ["3.0.1", "2026-09-13", 8, "Fixes the bug above with a real semver suffix on the I.P.U. build only (<code class='inline'>-IPU</code>), so the updater compares correctly and actually detects the stable release as newer.", "1 open, cosmetic: the \"ready to install\" screen showed the raw version (\"3.0.1-IPU\") instead of the proper label — closes in v3.0.2"],
  ["3.0.3", "2026-09-13", 8, "Fixes the stuck update banner and finishes clearing the old plain \"vX.Y.Z\" versioning out of every place that should already use the new one.", "1 open, scoped to power users: <code class='inline'>anchoran changeto</code> still didn't show or let you install I.P.U. releases — closes in v3.0.4"],
  ["3.0.4", "2026-09-13", 9, "<code class='inline'>anchoran changeto</code> now lists and installs I.P.U. releases (short names too, \"Build #H#.#\") and guards a downgrade past v2.9.2 behind a mandatory data-wipe confirmation; this very app is born, preinstalled and unremovable; Guest can no longer open the Terminal at all; the taskbar background is noticeably more translucent.", "no known open bugs at this point"],
  ...
  ["3.0.0", "2026-09-13", 7, "The two-stage system (I.P.U./stable) and the new \"Version # | Build #H#.#\" versioning are born — good new infrastructure.", "1 open, narrowly scoped: a device already on this version's I.P.U. build would never automatically receive the stable release — same version number, the updater saw them as equal (closes in v3.0.1)"],
  ["3.0.2", "2026-09-13", 7, "Fixes the raw label on the install screen and recategorizes the previous update's type from \"critical\" to \"stability\", a fairer fit.", "2 open: the pending-update banner could stay stuck on screen even after a check that said \"up to date\", and the GitHub release description still showed plain \"vX.Y.Z\" — both close in v3.0.3"],
